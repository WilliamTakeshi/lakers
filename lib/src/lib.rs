...
impl<Crypto: CryptoTrait> EdhocResponderProcessedM1<Crypto> {
    pub fn prepare_message_2(
        ...
        let method_details = match (self.state.method, &self.r) {
            (EDHOCMethod::SigSig, ResponderIdentity::SigSig { sk_r }) => {
                PrepareMessage2Details::SigSig {
                    sk_r,
                    cred_transfer,
                }
            }
    ...
    fn test_handshake_sigsig() {
        ...
        let responder = EdhocResponder::new(
            ...
            ResponderIdentity::SigSig {
                sk_r: R.try_into().expect("Wrong length of responder private key"),
            },
            cred_r.clone(),
        ); // has to select an identity before learning who is I
        ...
        initiator
            .set_identity(
                InitiatorIdentity::SigSig {
                    sk_i: I.try_into().expect("Wrong length of initiator private key"),
                },
                cred_i.clone(),
            )
            .unwrap(); // exposing own identity only after validating cred_r
