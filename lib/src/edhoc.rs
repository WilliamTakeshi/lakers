...
/// The Signature_or_MAC_2 and Signature_or_MAC_3 fields of RFC 9528: a MAC when the peer
/// authenticates with a static DH key, and a signature when it authenticates with a signature key.
#[derive(Debug)]
enum SignatureOrMac {
    Mac(BytesMac),
...
impl SignatureOrMac {
    /// Wrap the output of [`compute_mac_2`] or [`compute_mac_3`].
    ///
    ...
    fn new<const N: usize>(value: [u8; N]) -> Self {
        ...
        if N == MAC_LENGTH {
            SignatureOrMac::Mac(
                value[..]
                    .try_into()
                    .expect("const checked on the beginning of this fn"),
            )
        } else {
            SignatureOrMac::Signature(
                value[..]
                    .try_into()
                    .expect("const checked on the beginning of this fn"),
            )
...
fn compute_th_3(
    ...
    th_2: &BytesHashLen,
    plaintext_2: &BufferPlaintext2,
    // `cred_r` is only included for STATSTAT method where TH_3 is computed over
...
/// Encode the COSE_Sign1 Sig_structure whose signature is Signature_or_MAC_2 or
/// Signature_or_MAC_3, as defined in Sections 5.3.2 and 5.4.2 of RFC 9528:
///
...
fn encode_plaintext_2(
    ...
    if let Some((id_cred_r, sig_or_mac_2)) = stat_fields {
        ...
        plaintext_2
            .extend_from_slice(sig_or_mac_2)
            .or(Err(EDHOCError::EncodingError))?;
    }
...
mod tests {
    ...
    fn test_compute_th_4_psk() {
        let th_4 = compute_th_4(
            ...
            &CRED_I_PSK_TV,
            Th4Input::Psk {
                id_cred: &ID_CRED_PSK_TV,
                cred_r: &CRED_R_PSK_TV,
                ead_3: &EadItems::new(),
            },
        );
    ...
    fn test_encode_plaintext_2_psk() {
        let plaintext_2 = encode_plaintext_2(C_R_PSK_TV, None, &EadItems::new()).unwrap();
...
//
// FIXME:
// That trace uses cipher suite 0 (X25519 + EdDSA), which lakers does not implement YET. Every
// value in the trace from PRK_2e onwards, however, is produced by SHA-256, HKDF and
// AES-CCM-16-64-128 only -- identical in suite 0 and suite 2.
//
...
mod rfc9529_method0 {
    ...
    const SIG_OR_MAC_2: BytesSignature = hex!(
    ...
    const SIG_OR_MAC_3: BytesSignature = hex!(
    ...
    fn plaintext_2_matches() {
        let plaintext_2 = encode_plaintext_2(
            c_r(),
            Some((ID_CRED_R, &SignatureOrMac::new(SIG_OR_MAC_2))),
            &EadItems::new(),
        )
        .unwrap();
    ...
    /// The RFC trace identifies credentials by `x5t` (COSE header 34). `IdCred` only
    /// understands `kid` (4) and `kccs` (14), so the decode side of this trace is out of
    /// reach; the encode side above is still checked byte for byte. This is a limitation of
    /// lakers' credential support, not of the SIG-SIG state machine: the round-trip decode
    /// with a `kccs` identifier is covered by `test_handshake_sigsig`.
    #[test]
    ...
    fn plaintext_3_matches() {
        let plaintext_3 = encode_plaintext_3(
            Some((ID_CRED_I, &SignatureOrMac::new(SIG_OR_MAC_3))),
            &EadItems::new(),
        )
        .unwrap();
