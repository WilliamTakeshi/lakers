...
//! Shared support code for the nRF54L15 EDHOC example: a hand-rolled, register-level 2.4 GHz radio
//! driver (there is no `embassy-nrf` BLE driver for the nRF54L15), the on-air `Packet` framing, the
//! demo credentials, and the hardware-crypto constructor.
//!
//! The radio is programmed with the exact same on-air format the nRF52840 example uses
//! (`Mode::BLE_1MBIT`, S0/LENGTH framing, 3-byte base address + prefix, 24-bit CRC, BLE whitening),
//! so two nRF54L15 boards — one running the `initiator` binary, one the `responder` binary — can
//! talk to each other over the raw radio.
...
impl Packet {
    ...
    /// Rearranges `pdu` in place into the on-air layout `[S0][LENGTH][header?][payload…]` and
    /// returns a slice starting at `pdu[0]` (its pointer is what the radio DMA reads; the radio
    /// itself uses the LENGTH byte to decide how many bytes to send).
...
/// Blocking, register-level driver for the nRF54L15's 2.4 GHz radio, configured for the BLE 1 Mbit
/// PHY. Mirrors the API the nRF52840 example gets from `embassy-nrf`, but runs synchronously
/// (busy-waiting on the radio events) so the example needs no async executor.
pub struct Radio {
