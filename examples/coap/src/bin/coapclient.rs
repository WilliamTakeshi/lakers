...
fn client_handshake() -> Result<(), EDHOCError> {
    ...
    let (initiator, message_1) = initiator.prepare_message_1(Some(c_i), &EADItem::new_many())?;
    ...
    let (mut initiator, message_3, prk_out) =
        initiator.prepare_message_3(CredentialTransfer::ByReference, &EADItem::new_many())?;
