...
pub mod test_helper {
    ...
    /// Checks a backend's one-shot SHA-256 against the well-known `SHA-256("abc")` vector
    /// (FIPS 180-4 / RFC 6234).
    pub fn test_sha256_digest<C: Crypto>(crypto: &mut C) {
        let expected: BytesHashLen = [
            0xba, 0x78, 0x16, 0xbf, 0x8f, 0x01, 0xcf, 0xea, 0x41, 0x41, 0x40, 0xde, 0x5d, 0xae,
            0x22, 0x23, 0xb0, 0x03, 0x61, 0xa3, 0x96, 0x17, 0x7a, 0x9c, 0xb4, 0x10, 0xff, 0x61,
            0xf2, 0x00, 0x15, 0xad,
        ];
        assert_eq!(crypto.sha256_digest(b"abc"), expected);
    }
